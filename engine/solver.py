...
class Trainer(object):
    def __init__(self, config, args, model, dataloader, logger=None, teacher=None):
        ...
        self.progr_numsteps = config['solver']['progr_epochs'] #To be changed (similar to max epochs)
        self.batch = config['dataloader']['batch_size'] #Also added for the sampling step of the trainer. 
    ...
    def distill(self):          #Performs one step of progressive distillation 
        ...
        with tqdm(initial=step, total=self.progr_numsteps) as pbar:
            while step < self.progr_numsteps:
                total_loss = 0.
                for _ in range(self.gradient_accumulate_every):
                    data = next(self.dl).to(device) # get the data  
    ...
    def sample(self, num, size_every, shape=None):
        ...
        for _ in range(num_cycle):
            sample = self.ema.ema_model.generate_mts(batch_size=size_every)    #Generate mts is in the Gaussian Diffusion Module! 
